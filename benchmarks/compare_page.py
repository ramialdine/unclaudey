...
ARMS = (("baseline", "frontend-design only"), ("unclaudey", "unclaudey"))
...
.pair{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,520px),1fr));gap:22px}
...
def build(collect_dir: str, notes_path: str, out: str) -> None:
    ...
    parts = [
        ...
        f"<style>{CSS}</style>",
        '<div class="wrap">',
        '<header><h1>Same brief, same model. One of them <span class="circle">looked</span> at photos first.</h1>',
        '<p class="lede">Three briefs, each built twice by an independent Claude session: once with Anthropic’s '
        '<em>frontend-design</em> skill, once with <em>unclaudey</em> (the same guidance plus real, searched, reviewed photography).</p></header>',
    ...
    for bid, arms in facts.items():
        ...
        for arm, label in ARMS:
            ...
            parts.append(
                ...
                f'<button type="button" data-view="mobile" aria-pressed="false">Mobile</button></div>'
                ...
                f'{facts_dl(a["facts"])}<p class="note">{html.escape(n.get(arm, ""))}</p></figure>')
            frame_no += 1
        parts.append("</div>")
        if n.get("verdict"):
